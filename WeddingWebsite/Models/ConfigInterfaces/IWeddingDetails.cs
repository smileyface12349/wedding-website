...
public interface IWeddingDetails
{
    ...
    public IContactMethod? LoginContactMethod => GetContactMethod(ContactReason.Website);
    public IEnumerable<IContact> Contacts => NotablePeople.Concat(ExtraContacts);
    
    // Get the first contact method for a particular reason.
    public IContactMethod? GetContactMethod(ContactReason reason)
    {
        return NotablePeople.Concat(ExtraContacts)
                   .Select(p => p.ContactDetails.NotUrgent)
                   .FirstOrDefault(c => c.MatchesReason(reason))?.Methods.FirstOrDefault()
               ?? NotablePeople.Concat(ExtraContacts)
                   .Select(p => p.ContactDetails.Urgent)
                   .FirstOrDefault(c => c.MatchesReason(reason))?.Methods.FirstOrDefault();
    }
}